 ...
 * stores them — that is the whole point of the httpOnly migration.
 *
 *   - Send `credentials: 'include'` on every request so the
 *     cookies are attached even on cross-origin calls.
 *   - The 401 interceptor fires `onUnauthorized` (set by the auth
 *     provider), which tries `/auth/refresh` once. If refresh
 *     fails it clears the user state and redirects to /login.
 */
...
async function request<T>(
  ...
  const response = await fetch(`${BASE_URL}/api/v1${endpoint}`, {
    ...
    headers,
    body: fetchBody,
    // httpOnly cookies must be explicitly opted-in for cross-origin
    // requests. Same-origin in dev (localhost:3000 -> :8000) and
    // behind a reverse proxy in prod both need this flag.
    credentials: "include",
  });
  ...
  if (!response.ok) {
    ...
    if (response.status === 401 && onUnauthorized) {
      try {
        ...
        // The handler is best-effort; surface the original error too.
