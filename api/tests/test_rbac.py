...
async def _register(client: AsyncClient, *, email: str | None = None, password: str = "testpass123", name: str = "User") -> dict:
    ...
    data = resp.json()
    # Cookie-based auth: expose the vg_access token wrapped as
    # Authorization: Bearer for tests that still pass that header.
    data["headers"] = {"Authorization": f"Bearer {resp.cookies.get('vg_access')}"}
    return data
...
async def test_viewer_cannot_create_project(client: AsyncClient):
    ...
    viewer_token = joiner.cookies.get("vg_access")
